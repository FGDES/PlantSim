...
#
# must be run with the application folder as current dir (!!)

# test for application sources
if [ ! -f faudes.pri ]; then
  echo "distclean.sh: this is not an application source directory ... refusing to delete files"
  return
fi
